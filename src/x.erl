...
cache_lookup_atom(_Display, Atom) when is_integer(Atom) ->
    nyi;
cache_lookup_atom(_Display, Atom) when is_atom(Atom); is_list(Atom) ->
    nyi.

cache_insert_atom(_Display, _Atom, _N) ->
    nyi.

internAtom(Display, OnlyIfExist, Atom) when is_atom(Atom) ->
...
internAtom(Display, OnlyIfExist, Atom) when is_list(Atom) ->
...
getAtomName(Display, N) when is_integer(N) ->
...
drawLine(Display, D, Gc, X1, Y1, X2, Y2) ->
    xcli:send(Display, xproto:polyLine(D, Gc, X1,Y1,X2,Y2)).
...
listFonts(_Display, _Pattern, _MaxNames) ->
    nyi.
...
getKeyboardControl(Display) ->
	...
	Bin when is_binary(Bin) ->
	    {Rec, _Bin1} = ?xGetKeyboardControlReply_dec(Bin),
	    #xKeyboardState {
...
decode_keyboard_mapping(<<>>, [], Map, 0, _N) ->
    reverse(Map).
...
decode_modifier_mapping(<<>>, [], Map, 0, _N) ->
    reverse(Map).
