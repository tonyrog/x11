...
resolve_elems([],Acc,_Types,_Defines) ->
    {ok, reverse(Acc)}.
...
gen_macro_type({struct,{ident,TypeName},_Elems},Name,none,_,_Ts) ->
    "?" ++ macro_name(TypeName) ++ "(" ++ Name ++ ")";
gen_macro_type({union,{ident,TypeName},_Elems},Name,none,_,_Ts) ->
    "?" ++ macro_name(TypeName) ++ "(" ++ Name ++ ")";
gen_macro_type({union,none,_Elems},_Name,none,_,_Ts) ->
    "UNION";
gen_macro_type({ident,TypeName},Name,none,_,_Ts) ->
    "?" ++ macro_name(TypeName) ++ "(" ++ Name ++ ")";
gen_macro_type(['unsigned',T],Name,none,all,_Ts) ->
    "?" ++ atom_to_list(T) ++ "(" ++ 
	Name ++ ",unsigned,?ENDIAN)";
gen_macro_type(['signed',T],Name,none,all,_Ts) ->
    "?" ++ atom_to_list(T) ++ "(" ++ 
	Name ++ ",signed,?ENDIAN)";
gen_macro_type('float',Name,none,all,_Ts) ->
    "?" ++ "float(" ++ 
	Name ++ ",?ENDIAN)";
gen_macro_type('double',Name,none,all,_Ts) ->
    "?" ++ "double(" ++ 
	Name ++ ",?ENDIAN)";
gen_macro_type(['unsigned','int'],Name,none,N,_Ts) ->
    Name ++ ":" ++ integer_to_list(N) ++
	"/?ENDIAN-unsigned-integer";
gen_macro_type(['signed','int'],Name,none,N,_Ts) ->
    Name ++ ":" ++ integer_to_list(N) ++
	"/?ENDIAN-signed-integer";
gen_macro_type(['unsigned','char'],Name,N,all,_Ts) ->
    Name ++ ":" ++ integer_to_list(N) ++
	"/binary";
gen_macro_type(['signed','char'],Name,N,all,_Ts) ->
    Name ++ ":" ++ integer_to_list(N) ++
	"/binary";
