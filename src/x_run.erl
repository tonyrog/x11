...
print_key(Key,State,MinKeyCode, MaxKeyCode, KeyMap, _ModMap) ->
...
draw_rects(D, W, Gcs, WWidth, WHeight) ->
    X1 = rand:uniform(WWidth)-1,
    X2 = rand:uniform(WWidth)-1,
    Y1 = rand:uniform(WHeight)-1,
    Y2 = rand:uniform(WHeight)-1,
    {X,Width} = if X1 < X2 ->
			{X1, X2-X1};
		   true ->
			{X2, X1-X2}
    ...
    Gc = element(rand:uniform(size(Gcs)), Gcs),
...
draw_line(_D, _W, _Gcs, _X1, _X2, _Y) ->
    ok.
