...
auth(Creator,Host="localhost",Auth,DpyN,DefScr) ->
    Name = "/tmp/.X11-unix/X0",
    ?DBG("auth: ~p ~p ~p ~p\n", [Name, DpyN, DefScr,Auth]),
    case afunix:connect(Name,
...
setup(Creator,_Host,Socket,_DpyN,DefScr,Length32,MajorVersion, MinorVersion) ->
    ?DBG("setup: ~p ~p ~p\n", [_Host, _DpyN, DefScr]),
...
setup_screens(_Display, I, I, Data) -> Data;
...
setup_depths(_Display,N,N,Data,Depths) -> 
    {reverse(Depths),Data};
...
allocId_lookup(_Store, _ResId, _ResBase, _ResShift, _ResIMask, 0) ->
